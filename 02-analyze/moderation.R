...
for (sid in target_surveys()) {
  ...
  r <- list(interaction_p=summary(ml)$coefficients[4,4], coef=coef(summary(ml)), simple_slopes=list(female=c(b=unname(coef(mf)[2]),p=summary(mf)$coefficients[2,4]), male=c(b=unname(coef(mm)[2]),p=summary(mm)$coefficients[2,4])))
