...
# 调查二 清洗编码：文本→数值
# 输入: data/raw/问卷数据_精简版_206条.xlsx
# 输出: data/cleaned/survey2.rds

source("lib/encode.R")
source("lib/utils.R")

module_header("调查二 清洗编码")

df_raw <- load_survey("data/raw/问卷数据_精简版_206条.xlsx")
message(sprintf("原始: %d × %d", nrow(df_raw), ncol(df_raw)))

df_clean <- encode_survey2(df_raw)
...
saveRDS(df_clean, "data/cleaned/survey2.rds")
message("Saved: data/cleaned/survey2.rds")
